...
class Jaco():
    ...
    def getDynamicsInfo(self, jointId):
        dynamicsInfo = p.getDynamicsInfo(self.robotId, jointId)
        mass, friction, inertia, pos, rot, _, _, _, _, _, _, _ = dynamicsInfo
    ...
    def calcIK(self, pos=[1, 1, 1], rot=[np.pi/2, np.pi/2, np.pi/2]):
        jointAngle = p.calculateInverseKinematics(
            self.robotId,
            self.eeId,
            pos,
            rot,
    ...
    def reset(self, jointAngle=[
        ...
        for i in range(len(self.joint_id_list)):
            p.resetJointState(
                ...
                jointIndex=self.joint_id_list[i],
                targetValue=jointAngle[i],
            )
    ...
    def grip(self, grip=True):
        ...
        p.setJointMotorControlArray(
            ...
            jointIndices=[10, 12, 14],
            controlMode=p.POSITION_CONTROL,
            targetPositions=[targetPosition] * 3,
        )
    ...
    def setJointsAngle(self, jointAngle=[
        ...
        p.setJointMotorControlArray(
            bodyIndex=self.robotId,
            jointIndices=self.joint_id_list,
            controlMode=p.POSITION_CONTROL,
